...
begin
  ...
  ExtractStrings([#$D,#$A],[],Pchar(Value),ls);//将每行导入到字符串列表中

  for i :=0 to ls.Count-1 do
  begin
    if pos('序号:',ls[i])>0 then
    begin
      result:=stringreplace(ls[i],'序号:','',[rfReplaceAll,rfIgnoreCase]);
...
begin
  ...
  ReceiveItemInfo:=VarArrayCreate([0,ls.Count-1+5],varVariant);//+5:表示5个全血粘度项目
  ...
  begin
    ...
    if dlttype='全血粘度' then QXLD:=copy(sValue,1,pos(';',sValue)-1);
  end;
  ls.Free;
  ...
  begin
    if pos('1.00,',lsQXLD[j])>0 then ReceiveItemInfo[i]:=VarArrayof(['全血粘度1',stringreplace(lsQXLD[j],'1.00,','',[]),'','']);
    if pos('5.00,',lsQXLD[j])>0 then ReceiveItemInfo[i+1]:=VarArrayof(['全血粘度5',stringreplace(lsQXLD[j],'5.00,','',[]),'','']);
    if pos('50.00,',lsQXLD[j])>0 then ReceiveItemInfo[i+2]:=VarArrayof(['全血粘度50',stringreplace(lsQXLD[j],'50.00,','',[]),'','']);
    if pos('100.00,',lsQXLD[j])>0 then ReceiveItemInfo[i+3]:=VarArrayof(['全血粘度100',stringreplace(lsQXLD[j],'100.00,','',[]),'','']);
    if pos('200.00,',lsQXLD[j])>0 then ReceiveItemInfo[i+4]:=VarArrayof(['全血粘度200',stringreplace(lsQXLD[j],'200.00,','',[]),'','']);
  end;
  lsQXLD.Free;
